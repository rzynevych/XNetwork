...
public class AuthController {
    ...
    public ResponseEntity<String> authCheck() {
        if (UserHandler.isUserAuthorized())
            return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body("{\"status\" : true}");
        return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body("{\"status\" : false}");
    }
